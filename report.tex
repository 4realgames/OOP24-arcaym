...
\usepackage{alltt, fancyvrb, url, svg}
\usepackage{graphicx}
...
\title{
	\begin{figure}[H]
		\centering{}
		\includegraphics[width=\textwidth]{img/ARCHITECT-of-MAYHEM.png}
		\label{img:writing-worldshardestgame}
	\end{figure}
}

\author{
	Gioele Foschi \\
	\texttt{gioele.foschi@studio.unibo.it} \\[10pt]
	Nicholas Magi \\
	\texttt{nicholas.magi@studio.unibo.it} \\[10pt]
	Ludovico Maria Spitaleri \\
	\texttt{ludovico.spitaleri@studio.unibo.it} \\[10pt]
	Elettra Ventura \\
	\texttt{elettra.ventura@studio.unibo.it} \\
}
\date{\textsc{04 febbraio 2024}}
...
Il gioco "World's Hardest Game" consiste in diversi livelli "visti dall'alto" in cui una pallina, che parte da un punto A, deve raggiungere un punto B evitando diversi ostacoli.

\begin{figure}[H]
	\centering{}
	\includegraphics[width=\textwidth]{img/worlds-hardest-game.jpg}
	\caption{Riferimento al gioco 'The World's Hardest Game'}
	\label{img:worldshardestgame}
\end{figure}

L'applicativo mette a disposizione un editor che permette all'utente di creare, manipolare ed eliminare gli elementi che compongono un livello, selezionati da un gruppo di “prefabbricati”.
L'utente può salvare il progetto del livello in qualsiasi momento e, una volta soddisfatto, è in grado di giocarlo.

\subsubsection{Requisiti funzionali}
\begin{itemize}
	\item Realizzare un menu di selezione da cui l'utente può scegliere i vari elementi del livello in costruzione.
	\item Fornire uno spazio di piazzamento “tile-based” degli elementi.
	\item Garantire la possibilità di salvare lo stato di un livello in qualsiasi momento.
	\item Garantire la possibilità di giocare un livello in fase di creazione in qualsiasi momento.
	\item Predisporre di un sistema di punteggio minimale, basato su oggetti collezionabili generati all'interno del livello.
	\item Garantire l'adattamento dell'interfaccia utente al ridimensionamento della finestra.
	\item Una volta avviato un livello, il giocatore deve essere in grado di muoversi all'interno dello spazio designato dall'utente.
\end{itemize}

\subsubsection{Requisiti non funzionali}
\begin{itemize}
	\item Il gioco deve essere fluido (30 FPS minimi) e reattivo agli input dell'utente.
	\item L'applicazione deve essere compatibile con la maggior parte dei sistemi operativi esistenti.
	\item L'interfaccia grafica dovrà essere esteticamente gradevole e user-friendly.
\end{itemize}

\section{Modello del Dominio}
L'interfaccia di level editor dovrà fornire la possibilità di creare un livello modulare. Ogni oggetto piazzabile, opportunamente organizzato in un menu e posizionabile in una griglia, appartiene a una delle seguenti categorie:
%
\begin{itemize}
	\item \textbf{Blocchi}: oggetti basilari di costruzione per la definizione dello spazio (muro, pavimento, ecc…).
	\item \textbf{Ostacoli}: oggetti la cui presenza compromette il completamento del livello.
	\item \textbf{Collezionabili}: oggetti il cui contatto incrementa il punteggio.
\end{itemize}
%
Tra i \textbf{Blocchi} saranno presenti due zone particolari, una dedicata all'area di inizio del percorso e una rappresentante il traguardo. Inoltre, in fase di progettazione di un livello non sarà possibile piazzare il player — perché incorporato nella casella di partenza. \\
%
L'utente può passare in qualunque momento in modalità \textbf{gameplay}: il livello non è più modificabile, il player viene posizionato nel percorso - in attesa di input - e gli ostacoli “prendono vita”. 
%
% TODO: Inserire UML app domain.
% \begin{figure}[H]
% 	\centering{}
% 	\caption{Riferimento al gioco 'The World's Hardest Game'}
% 	\includegraphics[width=\textwidth]{img/arcaym-app-domain.png}
% 	\label{img:worldshardestgame}
% \end{figure}
